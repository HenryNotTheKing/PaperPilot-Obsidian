...
学术论文是一片密集的天空：一篇文章可能意味着几十页正文、上百条引用，以及多条交织的技术线索。

**Paper Pilot** 是你穿越这片天空的副驾驶（co-pilot）。

就像飞机副驾驶负责操控仪表盘、让机长专注于决策一样，Paper Pilot 负责阅读的机械性工作：导入 PDF、拆解章节、提取关键论断、把结论按类别高亮回写到原始页面，并从你的库或网络中找出相关论文。

你做决策，插件做导航。

这个名字也致敬了 *pilot study*——那种对陌生领域的第一次探索性阅读。Paper Pilot 正是为这种时刻而生：当你降落到一个全新的研究方向，需要迅速建立认知地图。

---

## 它能做什么

Paper Pilot 是一个面向 Obsidian 桌面端的论文阅读插件，不只是给你一段 AI 总结。

它导入 arXiv 论文后，按章节调用 LLM 提取关键句，再把这些句子按类型高亮回写到 PDF，让你的笔记能直接追溯到支撑结论的原始页面。
...
1. 从 [release 页面](https://github.com/HenryNotTheKing/PaperPilot-Obsidian/releases/latest) 下载 `main.js`、`manifest.json`、`styles.css`、`pdf.worker.min.mjs`。
...
| 配置项 | 说明 |
|---|---|
| Extraction model | 用于章节级提取的 LLM 接口与模型名称 |
| Summary model | 用于摘要生成的 LLM 接口与模型名称 |
...
| Highlight colors & opacity | 各类别的高亮颜色（动机、方法、结果……）及覆盖层透明度 |
